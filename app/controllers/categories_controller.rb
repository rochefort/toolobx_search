class CategoriesController < ApplicationController
  ...
  def show
    ...
    @tag_cloud_title = "#{@category.name.downcase} tags"

    # 上位10件のみチャートデータを取得
    @weekly_category_projects = @category.projects[0...10].map do |project|
      Project.for_a_week(project.name, project.project_link, @crawl_id)
    end

    @monthly_category_projects = @category.projects[0...10].map do |project|
      Project.for_a_month(project.name, project.project_link, @crawl_id)
    end
